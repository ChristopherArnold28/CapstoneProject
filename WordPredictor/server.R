...
library(quanteda)
loadFiles <- function(){
    wg1 <- get(load(file = "./data/WordGram1.RData"))
    wg2 <- get(load(file = "./data/WordGram2.RData"))
    wg3 <- get(load(file = "./data/WordGram3.RData"))
    wg4 <- get(load(file = "./data/WordGram4.RData"))
    wg5 <- get(load(file = "./data/WordGram5.RData"))
    wg6 <- get(load(file = "./data/WordGram6.RData"))
    ...
    wg1s <- get(load(file = "./data/WordGram1sw.RData"))
    wg2s <- get(load(file = "./data/WordGram2sw.RData"))
    wg3s <- get(load(file = "./data/WordGram3sw.RData"))
    wg4s <- get(load(file = "./data/WordGram4sw.RData"))
    wg5s <- get(load(file = "./data/WordGram5sw.RData"))
    wg6s <- get(load(file = "./data/WordGram6sw.RData"))
...
makeplotNStop <- function(input, wordFiles){
    allwords <- NGramPred
}

makeplotStop <- function(input, wordfiles){
    allwords <- NGramPrediction(input, wordfiles)
    axis_templatex <- list(title = "Probability of Word")
    axis_templatey <- list(title = "Word")
    #chosenWordsNStop <- data.frame(word = allwords$chosenWords[,length(colnames(allwords$chosenWords))-1], probability = allwords$chosenWords$probability)
    chosenwordsStop <- data.frame(word = allwords$chosenWordsSW[,length(colnames(allwords$chosenWordsSW))-1], probability = allwords$chosenWordsSW$probability)
    plot_ly(y = reorder(chosenwordsStop$word, chosenwordsStop$probability), 
            x = chosenwordsStop$probability, name = "Probability of predicted Words", 
            type = "bar", orientation = "h") %>%
        layout(xaxis = axis_templatex, yaxis = axis_templatey)
...
makeplotNStop <- function(input, wordfiles){
    ...
    chosenWordsNStop <- data.frame(word = allwords$chosenWords[,length(colnames(allwords$chosenWords))-1], probability = allwords$chosenWords$probability)
    #chosenwordsStop <- data.frame(word = allwords$chosenWordsSW[,length(colnames(allwords$chosenWordsSW))-1], probability = allwords$chosenWordsSW$probability)
    plot_ly(y = reorder(chosenWordsNStop$word, chosenWordsNStop$probability), 
            x = chosenWordsNStop$probability, name = "Probability of predicted Words", 
            type = "bar", orientation = "h") %>%
        layout(xaxis = axis_templatex, yaxis = axis_templatey)
...
shinyServer(function(input, output) {
   ...
   output$noStopWords<- renderPlotly({
       makeplotNStop(input$text, wordFiles)
   })
   
   output$courseraImage <- renderImage({
       outfile <- './www/coursera.png'
       list(src = outfile,
            contentType ='image/png',
            width = 300,
            height = 300)
   }, deleteFile = FALSE)
   output$swiftKeyImage <- renderImage({
       outfile <- './www/swiftkey.png'
       list(src = outfile,
            contentType = 'image/png',
            width = 250,
            height = 250)
